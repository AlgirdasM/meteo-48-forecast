...
weather_info = soup.find_all(
    'table', class_='hourly_weather')  # 48 hour forecast
...
for t in time_var:
    ...
    print(time_var[i] +
          '\tTemperature:' + temperature_var[i] +
          '\tFeels like:' + feellike_var[i] +
          '\tWind code:' + str(int(windcode_var)) +
          '\tWind from:' + str(int(wind_modulus)) +
          '\tWind speed:' + windspeed_var[i] +
          '\tWind gust:' + windgust_var[i] +
          '\tPrecipitation:' + precipitation_var[i] +
          '\tPressure:' + pressure_var[i] +
          '\tHumidity:' + humidity_var[i] +
          '\tCondition:' + condition_var[i])
    i += 1
